...
const Enrolled = () => {
    ...
    const userID = jwtDecode(localStorage.getItem('token')).sub.User_ID;
    const [hasFinal,setHasFinal] =useState(false);
  
    const fetchAveragePerSemester = async () => {
      try {
        const response = await Axios.post('http://127.0.0.1:5000/all_enrolled', { Student_ID: userID });
        if(response.data){
      ...
      try {
        const response = await Axios.post(
          ...
          { Semester: "Εαρινό Εξάμηνο 2024", User: userID}
        );
    ...
    const clickExportExcel = async (semesterData, filter,checkedFilterNames) => {
        try {
          ...
          const response = await Axios.post(`http://127.0.0.1:5000/enrolled_excel/${userID}`, {
    ...
    return (
      <div>
        <LoggedNavbar />
        <CustomButtonGroup  activePage='ΙΣΤΟΡΙΚΟ'/>
        <div className="boxgrey" style={{ height: 'your_box_height', marginBottom:"70px"}} ref={React.createRef()}>
  ...
  Εξαγωγή σε Excel <img src={excellogo} style={{ filter: 'invert(100%)', height: "24px", width: "30px", marginLeft: "10px" }} />
            ...
            >
              {!hasFinal && <Alert severity="warning">Εκκρεμεί οριστικοποίηση δήλωσης για το Εαρινό Εξάμηνο 2024</Alert>}
              <List>
                {semesterData ? (
                    <>
