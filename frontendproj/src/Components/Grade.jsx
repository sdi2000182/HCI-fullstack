...
const Grade = () => {
    ...
    const handleFilterChange = (newFilter) => {
      ...
      const clickExportExcel = async (semesterData, filter,checkedFilterNames) => {
        try {
          ...
          const response = await Axios.post(`http://127.0.0.1:5000/get_grades_excel/${userID}`, {
    ...
    const fetchAveragePerSemester = async () => {
      try {
        const response = await Axios.post('http://127.0.0.1:5000/average_per_semester', { Student_ID: userID});
    ...
    const hasFailedGrades = checkedFilterNames.length === 0 || checkedFilterNames.some((semesterName) => {
        ...
        if (semester) {
            ...
            console.log("ok", filteredSemesterGrades)
            return filteredSemesterGrades.length > 0;
        }
        return false;
    });
  ...
  >
    Μη Περασμένα
          ...
          <Button style={{ ...buttoninBoxStyles, marginLeft: "40px", marginTop:"20px" }} onClick={() => clickExportExcel(semesterData,filter,checkedFilterNames)}>
