...
const ClassesList = () => {
  ...
  const YourCardComponent = ({ subj ,reference}) => (
    <Card sx={{ ...cardStylessubj, left: 100, width: 720} } ref={reference}>
      ...
      <span style={{ ...textStyle3 }}>
        <Link to={`/courses/${subj.Class_ID}`} style={{ textDecoration: 'none', color: 'inherit' }}>
          Πληροφορίες
        </Link>
      </span>
  ...
  useEffect(() => {
    ...
    const getBottomPosition = (ref, refName) => {
      if (ref.current) {
        ...
        console.log(`${refName} Bottom Position: ${bottomPosition}px`);
    ...
    console.log("max is ",maxPosition);
    setBoxHeight(`${maxPosition + 50}px`)}
  ...
  <Divider aria-hidden="true" style={{ marginTop: "0", marginLeft: "2%", marginBottom: "20px", width: "42vw" ,alignSelf: 'flex-start'}} />
        ...
        <Route path="/ok" element={<YourComponent subj={subj} />} />
            ...
            <Search flag={0} setsubj={setsubj} />
            {!subj && <FilterComponent filter={filter} setFilter={setfilter} />}
          </div>
          {subj ? (
            
            <Class navigate={() => navigate(`/this_class/${subj.Name}/${subj.Class_ID}`)} />
