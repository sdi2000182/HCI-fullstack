...
import  Image1 from "./eudoxus.png";
...
const SubjectInfo = () => {
  ...
  const handlePencilClick = async () => {
    setIsEditing(true);
    setEditedData({
      Theory: subj[0]?.Theory || '',
      Lab: subj[0]?.Lab || '',
      WebPage: subj[0]?.WebPage || '',
    });
  };
  ...
  const handleSaveClick = () => {
    ...
    axios.post('http://127.0.0.1:5000/update_subject/Κ08', updatedData)
      .then(response => {
  ...
  <Divider aria-hidden="true" style={{ marginTop: "0", marginLeft: "2%", marginBottom: "20px", width: "42vw" ,alignSelf: 'flex-start'}} />
        ...
        >
          ...
          <Card
                ...
                >
                  <p style={{ ...textStyle }}> ΠΙΘΑΝΟΤΗΤΕΣ ΚΑΙ ΣΤΟΙΧΕΙΑ ΣΤΑΤΙΣΤΙΚΗΣ </p>
                  </Card>
